...
export default (() => {
  ...
  function prime(i, array) {
    ...
    return prime(
      i + 1,
      array.filter((x, index) => index === i || x % array[i] !== 0)
    );
  }
  let factorArray = prime(
    0,
    Array(1000)
      .fill(1)
      .map((x, i) => x * (i + 2))
  ...
  const randSign = (n = 1, weight = 0.0) => {
    return n > 1
      ? rng(n).map(x => (x > Math.round(5 + 10 * weight) ? '+' : '-'))
      : ['-', '+'][Math.round(Math.random() + weight)];
  };
  ...
  const nGen = (n = 1) => {
    return (exp = false) => {
      return exp
        ? {
            operator: randSign(),
            value: rng(1, 10 * Math.pow(n, n) + 10, 3 * Math.pow(n, n) - 1)
          }
        : {
            op: randSign(),
            value: rng(1, 10 * Math.pow(n, n) + 10, 3 * Math.pow(n, n) - 1)
  ...
  function compressable(num) {
    return {
      sameValueOp: num.nested.every(
        x => x.value === num.nested[0].value && x.op === num.nested[0].op
      ),
  ...
  function divide(n) {
    ...
    return factorize(n).includes(3)
      ? newArray
      : newArray.concat(n - 3 * newArray[0]);
  }

  function expand(num = false) {
    if (!num || num.value < 3) return num;
    return new Num(false, false, '+', num.op).addChild(
      ...divide(num.value).map(n => [n, false, '+', '+'])
    );
  }
  function compress(num = false) {
    if (!num || num.nested.length < 2) return num;
    if (compressable(num).sameValueOp) {
      if (num.nested[0].value === 1 && num.nested[0].op === '+')
        return new Num(num.nested.length, false, '+', num.op);
      let newNum = new Num(false, false, '+', num.op);
      if (num.nested[0].value === 1)
        return newNum.addChild([
          num.nested.length,
          false,
          '+',
          num.nested[0].op
      ...
      return newNum.addChild(
        [num.nested.length, false, '+', num.nested[0].op],
        [num.nested[0].value, false, '+', '*']
      );
    } else if (compressable(num).plusMinus) {
      let nFactor = factorize(
        num.nested.reduce((acc, val) => acc + val.value, 0)
      );

      return new Num(false, false, '+', num.op).addChild(
        [nFactor.pop(), false, '+', '+'],
        [nFactor.reduce((acc, val) => acc * val, 1), false, '+', '*']
      );
    } else return num;
  }
  return {
